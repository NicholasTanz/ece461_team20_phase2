...
describe('npm install command verification', () => {
    it('should correctly install all dependencies', () => {
        ...
        (0, chai_1.expect)(fs.existsSync(packageLockPath), 'package-lock.json should exist').to.be.true;
        ...
        const npmListOutput = (0, child_process_1.execSync)('npm list --depth=0 --json').toString();
        ...
        allDependencies.forEach(dependency => {
            (0, chai_1.expect)(installedPackages).to.have.property(dependency, `Dependency "${dependency}" should be installed`);
        });
        totalTests++;
        passedTests++;
...
describe('getBusFactor', () => {
    ...
    it('should return a bus factor score of 0.9 for 150 contributors', async () => {
        ...
        const busFactor = await (0, metrics_1.getBusFactor)('https://github.com/some/repo');
        (0, chai_1.expect)(busFactor).to.equal(0.9);
        totalTests++;
        if (busFactor == 0.9) {
            passedTests++;
        }
    ...
    it('should return a bus factor score of 0.3 for 3 contributors', async () => {
        ...
        const busFactor = await (0, metrics_1.getBusFactor)('https://github.com/some/repo');
        (0, chai_1.expect)(busFactor).to.equal(0.3);
        totalTests++;
        if (busFactor == 0.3) {
            passedTests++;
        }
    ...
    it('should return a bus factor score of 0.7 for 50 contributors', async () => {
        ...
        const busFactor = await (0, metrics_1.getBusFactor)('https://github.com/some/repo');
        (0, chai_1.expect)(busFactor).to.equal(0.7);
        totalTests++;
        if (busFactor == 0.7) {
            passedTests++;
        }
    ...
    it('should return -1 if the API call fails', async () => {
        sinon.stub(axios_1.default, 'get').rejects(new Error('API failed'));
        const busFactor = await (0, metrics_1.getBusFactor)('https://github.com/some/repo');
        (0, chai_1.expect)(busFactor).to.equal(-1);
        totalTests++;
        if (busFactor == -1) {
            passedTests++;
        }
...
describe('calculateRampUpMetric', () => {
    ...
    it('should calculate correct ramp-up score for a project with a README and code files', async () => {
        ...
        // Call the function
        const result = await (0, metrics_1.calculateRampUpMetric)(tempDir);
        // Make assertions
        (0, chai_1.expect)(result).to.be.greaterThan(0); // Ensure the score is greater than 0
        totalTests++;
        if (result > 0) {
            passedTests++;
        }
    ...
    it('should return 0 if no README is present', async () => {
        ...
        const result = await (0, metrics_1.calculateRampUpMetric)(tempDir);
        // Make assertions
        (0, chai_1.expect)(result).to.equal(0); // No README, so ramp-up score should be 0
        totalTests++;
        if (result == 0) {
            passedTests++;
        }
    ...
    it('should return similar ramp-up scores for files with 499 and 600 lines', async () => {
        ...
        const result499 = await (0, metrics_1.calculateRampUpMetric)(tempDir);
        // Remove the file
        ...
        const result600 = await (0, metrics_1.calculateRampUpMetric)(tempDir);
        // Make assertions: The ramp-up score for 499 and 600 lines should be very similar since the limit is 500
        (0, chai_1.expect)(result499).to.be.closeTo(result600, 0.01); // Allow a small margin for any slight differences
        totalTests++;
        passedTests++;
    });
    it('should handle a README with non-GitHub/NPM links', async () => {
        ...
        const result = await (0, metrics_1.calculateRampUpMetric)(tempDir);
        // Make assertions
        (0, chai_1.expect)(result).to.be.greaterThan(0); // Score should increase due to non-GitHub/NPM links
        totalTests++;
        if (result > 0) {
            passedTests++;
        }
...
describe('cloneRepo (real clone)', function () {
    ...
    it('should successfully clone a real repository', async () => {
        ...
        await (0, metrics_1.cloneRepo)(repoUrl, tempDir);
        ...
        (0, chai_1.expect)(dirExists).to.be.true; // Make sure the directory exists after cloning
        const files = fs.promises.readdir(tempDir);
        (0, chai_1.expect)((await files).length).to.be.greaterThan(0); // Make sure the directory isn't empty
        totalTests++;
        passedTests++;
    });
});
...
describe('checkLicenseCompatibility', () => {
    ...
    it('should return a compatible license from the LICENSE file', async () => {
        ...
        sinon.stub(axios_1.default, 'get').resolves(mockLicenseResponse);
        const result = await (0, metrics_1.checkLicenseCompatibility)('https://github.com/some/repo');
        (0, chai_1.expect)(result.score).to.equal(1);
        (0, chai_1.expect)(result.details).to.include('MIT License');
        totalTests++;
        if (result.score == 1) {
            passedTests++;
        }
    ...
    it('should return a compatible license from package.json', async () => {
        ...
        const result = await (0, metrics_1.checkLicenseCompatibility)('https://github.com/some/repo');
        (0, chai_1.expect)(result.score).to.equal(1);
        (0, chai_1.expect)(result.details).to.include('Apache-2.0');
        totalTests++;
        if (result.score == 1) {
            passedTests++;
        }
    ...
    it('should return 0 if no license information is found', async () => {
        ...
        const result = await (0, metrics_1.checkLicenseCompatibility)('https://github.com/some/repo');
        (0, chai_1.expect)(result.score).to.equal(0);
        (0, chai_1.expect)(result.details).to.equal('No license information found');
        totalTests++;
        if (result.score == 0) {
            passedTests++;
        }
    ...
    it('should handle GitHub token missing error', async () => {
        ...
        const result = await (0, metrics_1.checkLicenseCompatibility)('https://github.com/some/repo');
        (0, chai_1.expect)(result.score).to.equal(0);
        (0, chai_1.expect)(result.details).to.include('GitHub token not set');
        totalTests++;
        if (result.score == 0) {
            passedTests++;
        }
    ...
    it('should handle API failure gracefully', async () => {
        ...
        const result = await (0, metrics_1.checkLicenseCompatibility)('https://github.com/some/repo');
        (0, chai_1.expect)(result.score).to.equal(0);
        (0, chai_1.expect)(result.details).to.include('Error checking license: API failed');
        totalTests++;
        if (result.score == 0) {
            passedTests++;
        }
...
describe('calculateCorrectnessMetric', () => {
    ...
    it('should return 0 for a project with no test files or frameworks', async () => {
        ...
        const result = await (0, metrics_1.calculateCorrectnessMetric)('./mock/path');
    ...
    it('should return a score greater than 0 for a project with test files', async () => {
        ...
        const result = await (0, metrics_1.calculateCorrectnessMetric)('./mock/path');
        (0, chai_1.expect)(result).to.be.greaterThan(0);
        totalTests++;
        if (result > 0) {
            passedTests++;
        }
...
describe('calculateResponsiveMaintainerMetric', () => {
    ...
    it('should return 0 for a repository with no activity', async () => {
        sinon.stub(axios_1.default, 'get').resolves({ data: [] });
        const result = await (0, metrics_1.calculateResponsiveMaintainerMetric)('https://github.com/user/repo');
        (0, chai_1.expect)(result).to.equal(0);
        totalTests++;
        if (result == 0) {
            passedTests++;
    ...
    it('should return a score between 0 and 1 for a repository with mixed activity', async () => {
        ...
        const result = await (0, metrics_1.calculateResponsiveMaintainerMetric)('https://github.com/user/repo');
        (0, chai_1.expect)(result).to.be.within(0, 1);
        totalTests++;
        passedTests++;
    });
    it('should handle API errors gracefully', async () => {
        sinon.stub(axios_1.default, 'get').rejects(new Error('API error'));
        const result = await (0, metrics_1.calculateResponsiveMaintainerMetric)('https://github.com/user/repo');
        (0, chai_1.expect)(result).to.equal(0);
        totalTests++;
        if (result == 0) {
